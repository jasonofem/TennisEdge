...
export default function DashboardPage() {
  ...
  const stats = {
    ...
    avgEdge: predictions.length > 0 ? predictions.reduce((a: number, p: Prediction) => a + p.edge, 0) / predictions.length : 0,
    totalProfit: -1500,
    winRate: 72.5,
  };
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard title="Today's Picks" value={stats.todayPicks} suffix="/ 3" icon={<Target className="w-5 h-5" />} />
          <MetricCard title="Avg Edge" value={stats.avgEdge} suffix="%" decimals={1} trend={2.3} icon={<TrendingUp className="w-5 h-5" />} />
          <MetricCard title="Bankroll" value={bankroll.current} prefix="₦" icon={<DollarSign className="w-5 h-5" />} variant="success" />
          <MetricCard title="Win Rate" value={stats.winRate} suffix="%" decimals={1} trend={1.2} icon={<Activity className="w-5 h-5" />} />
        </div>
        ...
        {liveMatches > 0 && (
          <div className="flex items-center justify-between p-4 rounded-xl bg-black/60 border border-cyan-500/20 backdrop-blur-xl">
            ...
            <button onClick={loadData} className="p-2 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 hover:bg-cyan-500/20 transition-colors">
          ...
          {predictions.length > 0 ? (
            <div className="grid lg:grid-cols-1 xl:grid-cols-2 gap-4">
              {predictions.map((prediction, index) => (
                <PredictionCard key={prediction.id} prediction={prediction} bankroll={{ unitSize: bankroll.unitSize }} index={index} />
              ))}
            </div>
          ) : (
            <EmptyState title="NO QUALIFYING EDGE TODAY" description="System scan complete. No underdog opportunities meet the minimum edge threshold of 5%. Check back tomorrow for new analysis." variant="no-edge" icon={<Target className="w-8 h-8 text-cyan-400" />} />
          )}
        </div>

        <div className="grid lg:grid-cols-2 gap-6">
          <Card className="border-cyan-500/20 bg-black/60 backdrop-blur-xl">
            <CardHeader><CardTitle className="text-lg font-mono text-cyan-400 flex items-center gap-2"><DollarSign className="w-5 h-5" />Bankroll Status</CardTitle></CardHeader>
          ...
          <Card className="border-cyan-500/20 bg-black/60 backdrop-blur-xl">
            <CardHeader><CardTitle className="text-lg font-mono text-cyan-400 flex items-center gap-2"><Activity className="w-5 h-5" />System Status</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center p-3 rounded-lg bg-green-500/5 border border-green-500/20">
              ...
              <div className="flex justify-between items-center p-3 rounded-lg bg-purple-500/5 border border-purple-500/20">
                ...
                <span className="text-sm font-mono text-white">{lastUpdate.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-orange-500/5 border border-orange-500/20">
